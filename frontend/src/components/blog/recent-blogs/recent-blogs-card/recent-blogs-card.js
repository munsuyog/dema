...
const RecentBlogsCard = ({blog}) => {
  ...
  return (
    <Link href={`/blog/${blog.attributes.slug}`} className='recent-blogs-card'>
        <Image src={`http://154.53.59.178:30002${blog.attributes.featuredImage.data.attributes.url}`} width={355} height={172} alt='recent-blogs-card-image' className='recent-blogs-card-image' />
